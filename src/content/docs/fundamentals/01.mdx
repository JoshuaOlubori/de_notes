...
A foundational curriculum spanning concepts, patterns, failures, and decision frameworks that help junior data engineers progress to architects who can design systems that scale.
</Aside>

<Aside type="tip" title="The Core Thesis">
Data engineering a lot of times involve considering trade-offs under uncertainty. Every architectural decision involves sacrificing something: consistency for availability, latency for cost, flexibility for governance, simplicity for capability. Mastery comes from understanding *when* to make each trade-off, not from memorizing the "right" answer.
</Aside>
...
<CardGrid>
  <Card title="Batch vs Streaming" icon="rocket">
    Default to batch. Stream only when latency requirements *demand* it.
  </Card>
  <Card title="OLTP vs OLAP" icon="information">
    Use OLTP for transactions, OLAP for analysis. Never cross the streams.
  </Card>
  <Card title="ACID vs BASE" icon="setting">
    Financial transactions get ACID. User activity feeds can be BASE.
  </Card>
  <Card title="Centralized vs Federated" icon="puzzle">
    Start centralized. Federate when bottlenecks exceed coordination costs.
  </Card>
</CardGrid>
...
Data engineering exists because raw data holds little value and can cause issues if not properly processed. An unprocessed event stream, a corrupted JSON payload, a schema change that silently breaks downstream reports—these aren't edge cases. They're just another Tuesday for a data engineer.

The core mission is simple: **transform raw data into reliable, actionable information**. This involves:
...
At a fintech company, a currency conversion rate was accidentally inverted for 73 days. Every transaction report showed inverted profits/losses. The issue was discovered only when Q3 numbers "didn't feel right" to the CFO. By then, investor communications had been sent with incorrect figures.
</Aside>

#### Schema Drift

By default, upstream systems don't usually notify you when they change. That `user_id` field that was always an integer? Now it's sometimes a string UUID in the mobile app. Your pipeline continues running, casting to integer, and silently converting `"abc-123-def"` to `0`.

#### Pipeline Debt

The "quick fix" pipeline that was supposed to be temporary—three years ago—is now load-bearing infrastructure. It has no tests, no documentation, and the engineer who wrote it left the company. It processes 40% of your business data.
...
<Tabs>
  <TabItem label="What Stakeholders See" icon="approve-check">
    - Dashboards with charts
    - SQL tables they can query
    - APIs that return data
  </TabItem>
  <TabItem label="What They Don't See" icon="warning">
...
<CardGrid>
  <Card title="Reliability Over Features" icon="approve-check">
  ...
  <Card title="Validation Matters" icon="setting">
    Most pipeline failures stem from insufficient validation and observability
  </Card>
  <Card title="Infrastructure Work" icon="puzzle">
